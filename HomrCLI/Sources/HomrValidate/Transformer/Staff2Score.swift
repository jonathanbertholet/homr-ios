...
final class Staff2Score {
    ...
    func predict(_ image: GrayscaleImage) -> [EncodedSymbol] {
        ...
        do {
            FileHandle.standardError.write("  [predict] image \(image.width)x\(image.height), encoding…\n".data(using: .utf8)!)
            let context = try encoder.generate(x, shape: shape)
            FileHandle.standardError.write("  [predict] context shape \(context.shape), decoding…\n".data(using: .utf8)!)
            ...
            FileHandle.standardError.write("  [predict] decoded \(result.count) symbols\n".data(using: .utf8)!)
            return result
        } catch {
