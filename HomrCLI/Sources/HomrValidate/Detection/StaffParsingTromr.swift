...
private enum TromrInference {
    ...
    /// Serializes construction of `shared` only (not inference) so racing staff
    /// threads can't build the expensive model twice.
...
func predictBest(_ orgImage: GrayscaleImage, staff: Staff, config: TransformerConfig) -> [EncodedSymbol] {
    ...
    // outside it (a single ORT session is safe to `Run` concurrently).
