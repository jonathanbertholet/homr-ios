...
/// `instruments`, when supplied, identifies each part (one entry per voice, nil
/// where unknown); identified parts get a real name + GM program in the
/// `<part-list>` instead of the generic Voice/Piano default.
func generateXml(
    _ args: XmlGeneratorArguments, staffs: [[EncodedSymbol]], title: String,
    instruments: [Instrument?]? = nil
) -> MusicXmlDocument {
    MusicXmlDocument(root: generate_xml(args, staffs, title, instruments))
}
...
func generate_xml(
    ...
    root.addChild(build_part_list(has_two_staves_by_part, instruments))
...
/// Port of `build_part_list`, extended so an OCR-identified `Instrument`
/// overrides the generic Voice/Piano metadata for that part.
func build_part_list(_ has_two_staves_by_part: [Bool], _ instruments: [Instrument?]? = nil) -> XMLNode {
    ...
    for (part, has_two_staves) in has_two_staves_by_part.enumerated() {
        ...
        let instrument_name_str: String
        let instrument_sound_str: String
        let midi_program: Int
        let identified: Instrument? = {
            guard let instruments, part < instruments.count else { return nil }
            return instruments[part]
        }()
        if let identified {
            // Use the recognised instrument (1-based MIDI program for MusicXML).
            part_name_str = identified.name
            instrument_name_str = identified.name
            instrument_sound_str = identified.sound
            midi_program = identified.midiProgram1Based
        } else {
            (part_name_str, instrument_name_str, instrument_sound_str, midi_program) =
