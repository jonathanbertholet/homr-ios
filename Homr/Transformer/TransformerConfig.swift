...
final class TransformerConfig {
    ...
    /// CoreML (MLProgram) acceleration for the *encoder* (device only; the
    /// session factory falls back to CPU on the Simulator). Enabled: the encoder
    /// is a conv-heavy forward pass run once per staff, so GPU/ANE execution is a
    /// clear win. Cost: a one-time MLProgram compile (~tens of seconds) on the
    /// first scan after install; CoreML caches the compiled model on disk so
