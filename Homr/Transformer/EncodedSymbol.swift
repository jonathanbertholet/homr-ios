...
struct EncodedSymbol: Equatable, Hashable, Comparable, CustomStringConvertible, @unchecked Sendable {
    var rhythm: String
    var pitch: String
    var lift: String
...
/// Persist only the six semantic string fields (the cached duration and the
/// transformer `coordinates` are derived/transient). Decoding rebuilds a fresh
/// duration cache via the designated initializer. Enables storing per-staff
/// symbol streams in the on-disk score library.
extension EncodedSymbol: Codable {
    private enum CodingKeys: String, CodingKey {
        case rhythm, pitch, lift, articulation, slur, position
    }

    init(from decoder: Swift.Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.init(
            try container.decode(String.self, forKey: .rhythm),
            pitch: try container.decode(String.self, forKey: .pitch),
            lift: try container.decode(String.self, forKey: .lift),
            articulation: try container.decode(String.self, forKey: .articulation),
            slur: try container.decode(String.self, forKey: .slur),
    ...
    func encode(to encoder: Swift.Encoder) throws {
        ...
        try container.encode(articulation, forKey: .articulation)
        try container.encode(slur, forKey: .slur)
