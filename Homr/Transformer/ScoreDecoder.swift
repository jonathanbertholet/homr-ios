        ...
        do {
            ...
            for step in 0..<config.maxSeqLen {
                ...
                let keepGoing: Bool = try autoreleasepool { () -> Bool in
                    ...
                    // Pass nil runOptions: the arena-shrink config
                    // (`memory.enable_memory_arena_shrinkage=cpu:0`) was intended to
                    // cap RSS during the long autoregressive loop, but on iOS it can
                    // corrupt ORT's internal ObjC bridge and crash with an
                    // `OS_dispatch_mach_msg _setContext:` unrecognized-selector abort.
                    // Per-step autoreleasepool (above) already keeps memory flat.
